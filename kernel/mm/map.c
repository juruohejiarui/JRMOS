...
int mm_map_syncKrl() {
    if (task_cur->proc->mem.krlTblModiJiff.value != mm_map_krlTblModiJiff.value) {
        SpinLock_lock(&mm_map_krlTblLck);
        SpinLock_lock(&task_cur->proc->mem.pgTblLck);

        task_cur->proc->mem.krlTblModiJiff.value = mm_map_krlTblModiJiff.value;

	    int res = hal_mm_map_syncKrl();

        SpinLock_unlock(&task_cur->proc->mem.pgTblLck);
...
int mm_map(u64 virt, u64 phys, u64 attr) {
    ...
    else SpinLock_lock(&task_cur->proc->mem.pgTblLck);

    int res = hal_mm_map(virt, phys, attr);

    if (virt >= task_krlAddrSt) SpinLock_unlock(&mm_map_krlTblLck);
    else SpinLock_unlock(&task_cur->proc->mem.pgTblLck);
