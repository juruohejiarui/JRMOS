...
cpu_definevar(u32, hal_cpu_x2apic);
cpu_definevar(u32, hal_cpu_apic);
cpu_definevar(u8 *, hal_cpu_initStk);
cpu_definevar(u8 *, hal_cpu_cur);
cpu_definevar(u64, hal_cpu_flags);

__always_inline__ int _canEnableProc(u32 flag) {
	return (flag & 0x1) || (!(flag & 0x1) && (flag & 0x2));
}

static int _registerCPU(int idx, u32 x2apicId, u32 apicId) {
	// memset(desc, 0, sizeof(cpu_Desc));
	cpu_setCpuVar(idx, hal_cpu_x2apic, x2apicId);
	cpu_setCpuVar(idx, hal_cpu_apic, apicId);
	cpu_setCpuVar(idx, cpu_id, idx);
	cpu_setCpuVar(idx, cpu_bsAddr, cpu_bsAddr[idx]);
	// apic = apicId;
	if (x2apicId != hal_cpu_bspApicId) {
		...
		cpu_cpuPtr(idx, hal_intr_tss)->trIdx = _curTrIdx;
		_curTrIdx += 2;
		cpu_cpuPtr(idx, hal_intr_tss)->tss = mm_kmalloc(sizeof(hal_intr_TSS), mm_Attr_Shared, NULL);
		memset(cpu_cpuPtr(idx, hal_intr_tss)->tss, 0, sizeof(hal_intr_TSS));
		hal_intr_setTss(cpu_cpuPtr(idx, hal_intr_tss)->tss, 
			(u64)cpu_cpuPtr(idx, hal_cpu_initStk) + task_krlStkSize, 0, 0, 
			(u64)cpu_cpuPtr(idx, hal_cpu_initStk) + task_krlStkSize, 
			(u64)cpu_cpuPtr(idx, hal_cpu_initStk) + task_krlStkSize, 
			0, 0, 0, 0, 0);
		...
		cpu_cpuPtr(idx, hal_intr_tss)->trIdx = 10;
	}
	intr_initCpuVar(idx);
...
int _parseMadt() {
	...
	if (tmp) {
		for (u64 off = sizeof(hal_hw_uefi_MadtDesc); off < madt->hdr.length; ) {
			hal_hw_uefi_MadtEntry *curEntry = (hal_hw_uefi_MadtEntry *)((u64)madt + off);
			if (curEntry->tp == 9 && _canEnableProc(curEntry->tp9.flags))
				_registerCPU(tmp++, curEntry->tp9.x2apicId, curEntry->tp9.apicId);
			off += curEntry->len;
		}
	} else {
		tmp = 0;
		for (u64 off = sizeof(hal_hw_uefi_MadtDesc); off < madt->hdr.length; ) {
			hal_hw_uefi_MadtEntry *curEntry = (hal_hw_uefi_MadtEntry *)((u64)madt + off);
			if (curEntry->tp == 0 && _canEnableProc(curEntry->tp0.flags))
				_registerCPU(tmp++, curEntry->tp0.apicId, curEntry->tp0.apicId);
			off += curEntry->len;
		}
	}
...
int hal_cpu_init() {
	...
	for (int i = 0; i < cpu_num; i++) {
		printk(screen_log, "\t#%d: x2apic:%lx apic:%lx stk:%p idtTbl:%p idtPtr:%p tss:%p\n",
			i, 
			cpu_getCpuVar(i, hal_cpu_x2apic), cpu_getCpuVar(i, hal_cpu_apic), 
			cpu_getCpuVar(i, hal_cpu_initStk), cpu_cpuPtr(i, hal_intr_idt)->tbl, cpu_cpuPtr(i, hal_intr_idt),
			cpu_cpuPtr(i, hal_intr_tss)->tss);
	}

	cpu_bspIdx = cpu_num;
	for (int i = 0; i < cpu_num; i++)
		if (hal_cpu_bspApicId == cpu_getCpuVar(i, hal_cpu_x2apic)) {
			cpu_bspIdx = i;
			break;
		}
	if (cpu_bspIdx == cpu_num) {
	...
	// set gsbase
	hal_hw_writeMsr(hal_msr_IA32_GS_BASE, (u64)cpu_bsAddr[cpu_bspIdx]);
...
int hal_cpu_enableAP() {
	...
	*(u64 *)&icr = 0;
	...
	printk(screen_log, "cpu: wait for launching of APs.\n");
	...
	for (int i = 0; i < cpu_num; i++) if (i != cpu_bspIdx) {
		...
		hal_hw_apic_setIcrDest(&icr, cpu_getCpuVar(i, hal_cpu_apic), cpu_getCpuVar(i, hal_cpu_x2apic));
		hal_hw_apic_writeIcr(*(u64 *)&icr);

		hal_hw_apic_writeIcr(*(u64 *)&icr);

		while (cpu_getCpuVar(i, cpu_state) != cpu_Desc_state_Active) ;
	}
	printk(screen_log, "cpu: resume backup.\n");
	// copy it back
	...
	printk(screen_log, "cpu: clear backup.\n");
...
void hal_cpu_chk() {
	...
	cpu_setvar(hal_cpu_flags, flags);
