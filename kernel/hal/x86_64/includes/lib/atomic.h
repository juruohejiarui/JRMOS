...
__always_inline__ void hal_Atomic_add(hal_Atomic *atomic, i64 vl) {
	__asm__ volatile ( " lock addq %1, %0	\n\t"
		: "=m"(atomic->value) : "r"(vl) : "memory");
}

__always_inline__ void hal_Atomic_sub(hal_Atomic *atomic, i64 vl) {
...
__always_inline__ void hal_Atomic_bts(hal_Atomic *atomic, u64 vl) {
	__asm__ volatile ( " lock btsq %1, %0	\n\t"
		:  : "m"(atomic->value), "r"(vl) : "memory");
}

__always_inline__ void hal_Atomic_btc(hal_Atomic *atomic, u64 vl) {
	__asm__ volatile ( " lock btcq %1, %0	\n\t"
		: : "m"(atomic->value), "r"(vl) : "memory");
}

__always_inline__ void hal_Atomic_btr(hal_Atomic *atomic, u64 vl) {
